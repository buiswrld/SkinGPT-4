...
def main():
    ...
    update_predictions(diagnosis_informative, arrays["y_pred_informative"], total_images)
    ...
    # Iterate through the unique values and count occurrences
    for value in np.unique(arrays["skin_tone"]):
      if value in [1, 2, 3, 4, 5, 6]:
        skin_tone_counts[value] = np.count_nonzero(arrays["skin_tone"] == value)

    # Print the counts for each value
    for value, count in skin_tone_counts.items():
      print(f"Skin Tone {value}: {count} occurrences")

    indexes_equal_to_3 = [i for i, value in enumerate(arrays["skin_tone"]) if value == 4]
    print(indexes_equal_to_3)
    # Contact Dermatitis (Correct)
    metrics["Contact Dermatitis (Correct) (Between Groups)"], metrics["Contact Dermatitis (Correct) (To Overall)"] = calculate_fairness_metrics(
    ...
    # Eczema (Correct)
    metrics["Eczema (Correct) (Between Groups)"], metrics["Eczema (Correct) (To Overall)"] = calculate_fairness_metrics(
    ...
    metrics["Contact Dermatitis (Informative) (Between Groups)"], metrics["Contact Dermatitis (Informative) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][:split_index],
    ...
    # Eczema (Informative)
    metrics["Eczema (Informative) (Between Groups)"], metrics["Eczema (Informative) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][split_index:total_images],
    ...
    metrics["Contact Dermatitis (Helpful) (Between Groups)"], metrics["Contact Dermatitis (Helpful) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][:split_index],
    ...
    # Eczema (Helpful)
    metrics["Eczema (Helpful) (Between Groups)"], metrics["Eczema (Helpful) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][split_index:total_images],
    ...
    metrics["Contact Dermatitis (Understand) (Between Groups)"], metrics["Contact Dermatitis (Understand) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][:split_index],
    ...
    # Eczema (Understand)
    metrics["Eczema (Understand) (Between Groups)"], metrics["Eczema (Understand) (To Overall)"] = calculate_fairness_metrics(
        arrays["y_true"][split_index:total_images],
    ...
    hallucination_rate = calculate_hallucination_rate(hallucination_worksheet, total_entries=298)
    metrics["Hallucination Rate"] = hallucination_rate
    print(f"Final Hallucinations Rate: {hallucination_rate:.2f}")
